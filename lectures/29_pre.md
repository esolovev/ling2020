...
### 5. Еще задачка на оконные функции

При анализе поведения пользователей на сайтах и мобильных приложениях часто бывает полезным рассматривать не каждое отдельное посещение страницы, а так называемые пользовательские сессии - последовательность посещений сайта, произошедших за определённое время.

Представим себе, что у нас есть таблица visits, где каждая строчка - это посещение одним пользователем одной страницы, со следующими столбцами:

* ts: timestamp - время посещения
* url: varchar - страница, на которую зашел пользователь
* user_id: integer - id пользователя

Для удобства создайте такую табилицу у себя на сервере и заполните её небольшим количеством строк.

Назовём **сессией** последовательность визитов конкретного пользователя. Сессия обрывается, если между запросами прошло более 40 минут. Будем считать временем начала сессии время первого визита, а временем завершения сессии - (время последнего запроса + 40 минут).

То есть, если пользователь зашел на сайт в 00:10, 00:30, 01:00, 01:45, 01:50, он совершил две сессии: первая начинается в 00:10 и заканчивается в 01:40, вторая начинается в 01:45 и заканчивается в 02:30.

1. Напишите запрос, который добавляет в таблицу visits ID сессии и порядковый номер запроса в этой сессии. Иными словами, вам нужно получить таблицу с таким же количеством строк и следующими столбцами:

* session_id: varchar - id сессии, строится как `user_id + "_" + время начала сессии` (например, что-то вида `100500_2020-01-15_00:30`). Нам нужно придумать какой-то уникальный идентификатор сессии, и склейка id пользователя и времени начала сессии как раз дает нам уникальность.
* user_id: integer - пользователь, чья это сессия
* visit_number: integer - номер запроса в сессии (1, 2, 3, ...)
* ts
* url


2. Соберите из таблицы, полученной на этапе 1, таблицу, в которой столько строк, сколько всего уникальных сессий, и следующими столбцами:


* session_id: varchar - id сессии
* session_start_timestamp: timestamp - время начала сессии
* session_finish_timestamp: timestamp - время начала сессии
* user_id - пользователь, чья это сессия
* visits_count - число визитов в сессии
* visits_frequency - число визитов в минуту в этой сессии (`кол-во визитов в сессии / длина сессии во времени`). Например, если у нас есть сессия из двух запросов в 00:10 и 00:30, эта сессия завершается в 01:10, её длина составляет 60 мин, а частота приблизительно равна 0.033)
* urls: varchar - JSON-строка вида `["http://myurl.com/first_page","http://myurl.com/second_page"]` со всеми посещенными URL'ами в порядке посещения (вам пригодятся функции array_to_json и array_agg)


### 6. Дополнительные задачи

Еще можете порешать на сайте https://pgexercises.com/ задачи по следующим разделам:
